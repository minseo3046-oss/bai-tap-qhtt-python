...
def trich_xuat_dinh(bang, co_so, so_bien_goc=2):
    # Khởi tạo tọa độ các biến gốc (x1, x2, ...) bằng 0
    dinh = [0.0] * so_bien_goc
    
    for i, vi_tri_co_so in enumerate(co_so):
        # Nếu chỉ số biến cơ sở nằm trong phạm vi các biến gốc (x1, x2, ...)
        if vi_tri_co_so < so_bien_goc:
            # Giá trị biến cơ sở = Vế phải của dòng tương ứng
...
def kiem_tra_xoay_vong(lich_su_co_so, co_so_hien_tai):
    # Sort để đảm bảo tính duy nhất của tập hợp, sau đó chuyển thành tuple để hash
...
def thuat_toan_hai_pha(ma_tran_A, ve_phai_b, he_so_c):
    ...
    # Giải Pha 1
    ket_qua_pha_1, so_buoc1= chay_vong_lap_don_hinh(bang, co_so, dung_quy_tac_bland=False, lich_su_dinh=duong_di_toa_do, so_bien_goc=so_bien, lich_su_co_so=lich_su_co_so)
    ...
    if so_bien in co_so: # Nếu biến giả vẫn nằm trong cơ sở
        ...
        if bang[dong_chua_x0, -1] > 1e-9:
            ...
            return None, None, "vo_nghiem", duong_di_toa_do, so_buoc1
        else:
    ...
    # Giải Pha 2
    ket_qua_pha_2, so_buoc_2 = chay_vong_lap_don_hinh(bang, co_so, dung_quy_tac_bland=False, lich_su_dinh=duong_di_toa_do, so_bien_goc=so_bien, lich_su_co_so=lich_su_co_so)
    
    if ket_qua_pha_2 == "khong_gioi_noi":
        duong_di_toa_do=[]
        return None, float('-inf'), "khong_gioi_noi", duong_di_toa_do, so_buoc_2
    ...
    # Lưu dưới dạng danh sách các đỉnh (mỗi đỉnh là 1 tuple để dễ so sánh)
    dinh_toi_uu = [tuple(np.round(nghiem_A, 6))] 
    trang_thai = "toi_uu_duy_nhat"

    # --- TÌM TẤT CẢ CÁC ĐỈNH TỐI ƯU (VÔ SỐ NGHIỆM) ---
    so_cot_hien_tai = bang.shape[1] - 1
    for j in range(so_cot_hien_tai):
        # Nếu cột j không trong cơ sở và hệ số đánh giá bằng 0 -> Cạnh tối ưu
        if j not in co_so and abs(bang[-1, j]) < 1e-9:
            ...
            if ty_so_min != float('inf'):
                # Tính đỉnh B (đỉnh kề trên cạnh tối ưu) mà không cần xoay bảng
...
def thuat_toan_bland(ma_tran_A, ve_phai_b, he_so_c):
    ...
    trang_thai, so_buoc_lap = chay_vong_lap_don_hinh(bang, co_so, dung_quy_tac_bland=True, lich_su_dinh=duong_di_toa_do, so_bien_goc=so_bien, lich_su_co_so=lich_su_co_so)
    
    if trang_thai == "khong_gioi_noi":
        duong_di_toa_do=[]
        return None, float('-inf'), "khong_gioi_noi", duong_di_toa_do, so_buoc_lap
    ...
    # Trả về danh sách các đỉnh tối ưu
    nghiem_tra_ve = [np.array(d) for d in dinh_toi_uu]
    return nghiem_tra_ve, gia_tri_toi_uu, trang_thai, duong_di_toa_do, so_buoc_lap
...
def thuat_toan_dantzig(ma_tran_A, ve_phai_b, he_so_c):
    ...
    trang_thai , so_buoc_lap = chay_vong_lap_don_hinh(bang, co_so, dung_quy_tac_bland=False, 
    ...
    if trang_thai == "xoay_vong":
        return None, None, "xoay_vong",[], so_buoc_lap
